...
func main() {
	...
	l, err := model.NewLog("./log", model.Config{})
	if err != nil {
		log.Fatalf("initialize error, %v", err)
	}
	progLogSvc := proglogapi.NewProgLog(logger, &proglogapi.Config{
		CommitLog:  l,
		Authorizer: proglogapi.NewAuthorizer(config.ACLModelFile, config.ACLPolicyFile),
	})
	...
	case "local":
		{
			...
			go handleGRPCServer(ctx, u, progLogEndpoints, &wg, errc, logger, *dbgF)
		}
